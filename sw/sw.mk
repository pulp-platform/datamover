...
# JSON mode: take the named test from a suite
...
$(SW_HEADER): datamover_model/workloads/cli.py datamover_model/golden_model/transforms.py | $(SW_BUILD_DIR)
	python -m datamover_model.workloads.cli \
	    $(GEN_HEADERS_ARGS) \
	    $(if $(filter 1,$(NO_DEBUG)),--no-debug,) \
	    --output_dir $(SW_BUILD_DIR)

# Step 2: compile startup and test driver (HAL is header-only)
...
# Step 3: link
$(SW_BIN): $(SW_CRT) $(SW_OBJ) sw/link.ld
	$(RISCV_LD) $(LD_OPTS) -o $(SW_BIN) $(SW_CRT) $(SW_OBJ) -Tsw/link.ld
...
sw-clean:
	rm -f $(SW_OBJ) $(SW_CRT) $(SW_BIN) $(SW_BIN).s19 $(SW_BIN).txt $(SW_BIN).dump
